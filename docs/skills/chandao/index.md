...
title: 禅道
name: chandao
description: 通过脚本操作禅道项目管理系统。用户需要查询、创建或更新项目、产品、用户、任务、缺陷，或通过自然语言管理项目流程时使用。
metadata:
  author: Tnnevol
  version: "2026.05.29"
---
...
1. **禁止暴露** `CHANDAO_ACCOUNT` 或 `CHANDAO_PASSWORD`，包括聊天、文件、代码和日志。
2. **所有接口调用**必须通过脚本完成，禁止直接调用禅道接口。
3. **禁止读取并输出** `.env` 文件或包含凭据的环境变量。
...
| 主题           | 描述                       | 参考文档                                                           |
| -------------- | -------------------------- | ----------------------------------------------------------------- |
| 环境配置       | 安装与环境配置             | [setup.md](references/setup.md)                                   |
| 接口避坑       | 禅道接口踩坑记录（23 条）  | [pitfalls.md](references/pitfalls.md)                             |
| 帮助与问答     | 常见问题解答               | [help.md](references/help.md)                                     |
| 接口字段       | 第二版接口必填参数速查     | [zentao-v2-api-fields.md](references/zentao-v2-api-fields.md)     |
| 接口差异       | 接口第二版常见坑点         | [zentao-api-v2-quirks.md](references/zentao-api-v2-quirks.md)     |
| 权限配置       | 权限与角色问题             | [zentao-api-permissions.md](references/zentao-api-permissions.md) |
| 产品接口       | 产品管理命令               | [commands-product.md](references/commands-product.md)             |
| 项目接口       | 项目管理命令               | [commands-project.md](references/commands-project.md)             |
| 需求接口       | 需求管理命令               | [commands-story.md](references/commands-story.md)                 |
| 任务接口       | 任务管理命令               | [commands-task.md](references/commands-task.md)                   |
| 执行接口       | 执行与迭代管理命令         | [commands-execution.md](references/commands-execution.md)         |
| 缺陷接口       | 缺陷管理命令               | [commands-bug.md](references/commands-bug.md)                     |
| 测试接口       | 测试用例管理命令           | [commands-test.md](references/commands-test.md)                   |
