...
pub fn handle_execution(
    ...
    match cmd {
        ExecutionCommands::List {
            ...
            let status_val = status.as_deref().unwrap_or("undone");
            let order_val = order_by.as_deref().unwrap_or("rawID_asc");
            let mut path = format!(
                "/executions?status={}&orderBy={}&pageID={}&recPerPage={}",
                status_val, order_val, page, limit
            );
            if let Some(p) = project {
                path = format!(
                    "/projects/{}/executions?status={}&orderBy={}&pageID={}&recPerPage={}",
                    p, status_val, order_val, page, limit
                );
            }
            let data = ac.get(&path)?;
            utils::print_list(
                &data,
                &["id", "name", "status", "begin", "end", "projectName"],
                Some(*limit),
            );
            Ok(())
        }),
        ...
        ExecutionCommands::Create {
            ...
            with_auth!(client, auth, |ac: &mut AuthenticatedClient| {
                ...
                if let Some(l) = lifetime {
                    body["lifetime"] = json!(l);
                }
                if let Some(d) = days {
                    body["days"] = json!(d);
                }
                if let Some(p) = products {
                    body["products"] = json!(p);
                }
        ...
        ExecutionCommands::LinkProducts {
            id,
            products,
            plans,
            dry_run,
