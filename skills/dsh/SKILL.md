...
name: dsh
description: >-
  Use when 用户需要安装、运行、配置、排错或扩展 DeepSeek Harness（dsh），包括 profile 组合、Web 界面、无头任务、插件、Cordis、工具、模型适配器、会话和能力 seam。
metadata:
  author: Tnnevol
  version: "2026.08.15"
---
...
- 用户需要运行 Web 界面、一次性无头任务、源码版本或 Python SDK。
- 用户需要添加工具、模型适配器、服务、事件、会话节点、文件系统或其他可替换能力。
- 用户需要解释 agent（智能体）轮次、工具流水线、会话日志、配置覆盖、插件生命周期或排错方法。
...
5. **生命周期必须可逆。** 通过 ctx.on()、注册表或 ctx.effect() 建立的资源要能在插件卸载、HMR 和退出时清理；异步清理必须等待完全停稳。
6. **不要凭记忆猜配置和类型。** 优先运行 --help、--dump-config，再查看当前分支的配置目录、子系统页面或源码。

## 扩展开发工作流

1. 明确需求是运行配置、用户级插件，还是仓库内新包。
2. 用架构与运行时把需求映射到服务、事件、工具、会话或 UI seam。
...
- 运行时行为异常：先查看 session/event 持久日志，再区分实时 agent/* 事件、工具 tools/* 事件和模型 llm/stream 事件。
