...
async function actionUpdate(callAPI, argList) {
  ...
  if (!newContent && !visibility) {
    console.error("错误: 至少需要提供新内容或 --visibility 参数");
    process.exit(1);
  }
  ...
  // 如果没有提供新内容，先获取当前笔记内容
  if (!newContent) {
    const currentMemo = await callAPI("GET", `/api/v1/${id}`);
    newContent = currentMemo.content || "";
  }
  ...
  const url = `/api/v1/${id}?updateMask=${updateMask.join(",")}`;
